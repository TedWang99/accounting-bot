...
function parseReceiptWithGeminiVision(base64Image, mimeType = 'image/jpeg') {
  try {
    ...
    2. 金額處理：請只提取「最終總計金額」的純數字。⚠️ 特別注意：若收據上有「銷貨退回」、「折讓證明單」、「RETURN」、「Refund」等退貨/退款字眼，金額請務必回傳為負數（例如 -234733），以表示資金的流回。
    3. 日期推算：今天是 ${todayStr}，請根據收據上的資訊填寫正確的消費日期。
    4. 品名摘要：如果收據上有很長的多個品名，請用逗號隔開列出前 2-3 個主要項目即可。若為退款，品名請加上「(退款)」字樣。
    5. 商場明細特別處理：若看到「交易明細表」或類似字眼，請優先提取上方真正的品牌/專櫃名稱（例如：カルディ/咖樂迪），並將下方列出的品名合併為摘要，忽略商場或百貨公司本身的名字（如啦啦寶都、三井）。
    6. 分類與專案判斷：
       分類清單：${categoryList}
       若為退貨/退款/折讓，分類請強制設為「折扣回饋」或「收入」（依據退款性質）。
       專案清單：\n${projectList}
    7. [v7.7] 卡片資訊：
